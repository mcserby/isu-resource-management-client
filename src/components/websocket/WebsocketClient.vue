...
<script>
  ...
  export default {
    ...
    methods: {
      connect() {
        this.socket = new SockJS("http://localhost:8787/resources-ws");
        ...
        this.stompClient.connect(
          ...
          frame => {
            this.connected = true;
            console.log(frame);
            this.stompClient.subscribe("/topic/subunits", notification => {
              console.log(notification);
              this.received_messages.push(JSON.parse(notification.body).content);
            });
          },
          error => {
            console.log(error);
